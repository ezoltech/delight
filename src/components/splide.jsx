...
const slides = [
    {
        head: "bosko lorem henok did this ",
    ...
    },
    {
        ...
        imgurl: "https://www.welcome-hotels.com/site/assets/files/35059/welcome_hotel_marburg_lobby_2k.2560x1600.jpg",
                    ...
                    return (
                        <SplideSlide key={index}>
                            <div className='w-full text-center bg-[#53bcce18] overflow-hidden rounded-[3rem]' >
                                <img className='object-cover w-full md:h-[80vh] h-[50vh] rounded-t-[3rem]' src={slide.imgurl} alt="" />
                                <h1 className="md:text-[3rem] text-[2.6rem] text-[#fba61b] font-curley uppercase font-bold">
                                    {slide.head.split(' ')[0]} <span className='text-[#02b1b4] font-mont tracking-tighter '>{slide.head.split(' ').slice(1).join(' ')}</span>
                                </h1>
                                <p className="relative px-6 my-3 leading-[1.2] font-medium text-[#555] text-md">{slide.text}</p>
                            </div>
                        </SplideSlide>
                    )
                })}
